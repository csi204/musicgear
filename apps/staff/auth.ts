...
export const { handlers, auth, signIn, signOut } = NextAuth({
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // STRICT LOGIN: Staff only
          if (user.role !== "staff" && user.role !== "admin") {
            return null; // Block non-staff (admins can login to staff)
          }
  ...
  cookies: {
    sessionToken: {
      name: process.env.NODE_ENV === "production" ? "__Secure-mg_staff_session" : "mg_staff_session",
