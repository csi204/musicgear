...
import { getBundles, getInventory, getProducts, deleteBundle, BundleRecord, InventoryRecord } from "@/lib/api";
...
const statusConfig: Record<BundleStatus, { label: string; badge: string; dot: string; icon: React.ReactNode }> = {
  ...
  low_component: {
    ...
    badge: "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-blue-400",
    dot: "bg-blue-600",
...
function CreateBundleModal({ onClose, onSuccess }: CreateBundleModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl max-w-4xl w-full shadow-2xl animate-in zoom-in duration-200 flex flex-col max-h-[90vh] text-zinc-900 dark:text-zinc-100">
        ...
        <div className="px-6 py-5 space-y-5 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 block mb-1.5">ชื่อ Bundle <span className="text-red-500">*</span></label>
            ...
            <div>
              ...
              <CustomSelect
                ...
                onChange={(val: string) => setForm(prev => ({ ...prev, discountType: val }))}
                options={[
            ...
            <div>
              <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 block mb-1.5">มูลค่าส่วนลด <span className="text-red-500">*</span></label>
            ...
            <div className="flex gap-2 items-end">
              ...
              <div className="w-20">
                ...
                <input type="number" min={1} value={currentQty}
                  ...
                  className="w-full px-3 py-2 text-xs rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30 text-center font-bold" />
              </div>
          ...
          {success && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-emerald-50 dark:bg-blue-500/10 border border-emerald-200 dark:border-blue-500/20 text-emerald-700 dark:text-blue-400 text-sm">
        ...
        <div className="px-6 py-5 border-t border-zinc-200 dark:border-zinc-800 flex gap-3 bg-zinc-50 dark:bg-zinc-900/50">
          ...
          <button onClick={handleSubmit} disabled={isSaving || success}
            className="flex-1 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-bold transition-colors shadow-md shadow-blue-600/20 flex items-center justify-center gap-2">
            ...
            <div className="flex gap-2 items-end">
              <div className="flex-1">
                ...
                <CustomSelect
                  ...
                  options={[
                    { value: "", label: "-- เลือกสินค้า --" },
                    ...productList.map((p: any) => ({ value: p.productId, label: `${p.name} (${p.sku})` }))
                  ]}
                  triggerClassName="bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 border-zinc-200 dark:border-zinc-700 py-2.5 h-10 text-xs"
                  dropdownClassName="bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100 divide-y divide-zinc-100 dark:divide-zinc-800"
                />
...
export default function BundlesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const bundleRes = await getBundles(token);
      const invRes = await getInventory(token).catch(() => ({ inventories: [] }));
      
      const invMap = new Map((invRes.inventories ?? []).map((i: InventoryRecord) => [i.productId, i]));
      const mapped: Bundle[] = (bundleRes.bundles ?? []).map((b: any) => {
        const components: BundleComponent[] = (b.items ?? []).map((item: any) => {
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5">
        ...
        {isAdmin && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-colors shadow-md shadow-blue-600/20"
          >
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="relative group overflow-hidden rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-7 shadow-sm hover:shadow-md transition-all duration-300 min-h-[160px] flex flex-col justify-between">
              <div className="flex items-center justify-between">
                <span className="text-[15px] font-bold text-zinc-500 dark:text-white tracking-[0.7px]">Bundle ทั้งหมด</span>
                <div className="w-14 h-14 rounded-2xl bg-blue-50 dark:bg-white/10 flex items-center justify-center shrink-0">
                  <Package className="w-6 h-6 text-zinc-900 dark:text-white" />
                </div>
              </div>
              <div className="mt-5">
                <p className="text-5xl font-black text-zinc-900 dark:text-white tracking-tight leading-none">{totalActive}</p>
                <p className="text-xs text-zinc-500 dark:text-white mt-2 font-medium">รายการ Bundle ทั้งหมดในระบบ</p>
              </div>
            </div>

            <div className="relative group overflow-hidden rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-7 shadow-sm hover:shadow-md transition-all duration-300 min-h-[160px] flex flex-col justify-between">
              <div className="flex items-center justify-between">
                <span className="text-[15px] font-bold text-zinc-500 dark:text-white tracking-[0.7px]">พร้อมประกอบ</span>
              ...
              <div className="mt-5">
                ...
                <p className="text-xs text-zinc-500 dark:text-white mt-2 font-medium">มีสินค้าส่วนประกอบเพียงพอ</p>
              </div>
            </div>

            <div className="relative group overflow-hidden rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-7 shadow-sm hover:shadow-md transition-all duration-300 min-h-[160px] flex flex-col justify-between">
              <div className="flex items-center justify-between">
                <span className="text-[15px] font-bold text-zinc-500 dark:text-white tracking-[0.7px]">วัตถุดิบไม่พอ / มีปัญหา</span>
              ...
              <div className="mt-5 flex flex-col justify-end">
                <div className="flex items-baseline gap-2">
                  ...
                  <p className="text-xs text-zinc-500 dark:text-white font-bold">รายการ</p>
                </div>
                <p className="text-xs text-zinc-500 dark:text-white mt-2 font-medium">ขาดสินค้าวัตถุดิบบางรายการ</p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="relative flex-1 sm:flex-none">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 text-sm rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 w-full sm:w-60"
                  />
                </div>
                <button
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            {isAdmin && (
                              <button
                                ...
                                className="px-3 py-1.5 text-sm font-bold rounded-lg bg-blue-600 hover:bg-blue-700 text-white transition-colors"
                              >
                                แก้ไข
                              </button>
                            )}
...
function BundleDetailModal({ bundle, onClose, onSuccess }: BundleDetailModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(null);
      const token = getAccessToken();
      const res = await deleteBundle(bundle.id, token || undefined);
      if (res.status !== "ok") {
        throw new Error((res as any).message ?? "Failed to delete bundle");
      }
      onSuccess();
      onClose();
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl max-w-4xl w-full shadow-2xl animate-in zoom-in duration-200 overflow-hidden text-zinc-900 dark:text-zinc-100 flex flex-col max-h-[90vh]">
        {/* Header */}
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[12px] font-bold px-2 py-0.5 rounded-full ${
                bundle.status === "healthy" ? "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400" :
                bundle.status === "low_component" ? "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-blue-400" :
                      ...
                      <tr key={comp.sku} className="hover:bg-zinc-100/50 dark:hover:bg-zinc-800/20">
                        ...
                        <td className="py-2.5 px-4 text-right pr-4">
                          <span className={`font-bold px-2 py-0.5 rounded text-[12px] ${
                            sufficient ? "bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-blue-400" :
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-zinc-50 dark:bg-zinc-800/30 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 text-center">
              ...
              <p className={`text-2xl font-black mt-1 ${bundle.assembled === 0 ? "text-red-500" : "text-blue-500"}`}>
